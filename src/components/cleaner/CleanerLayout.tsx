...
import { User, Menu, Home, CheckSquare, LogOut, Phone, Calendar, ClipboardList } from 'lucide-react';
...
export function CleanerLayout({ children }: CleanerLayoutProps) {
  ...
  const fullName = user?.user_metadata?.full_name || 'Cleaner';
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const menuItems = [
    { icon: Home, label: 'Přehled', path: '/cleaner/dashboard' },
    { icon: Calendar, label: 'Historie', path: '/cleaner/history' },
    { icon: CheckSquare, label: 'Dnešní úkoly', path: '/cleaner/dashboard' },
  ];
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-900/50">
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="relative h-16 w-full max-w-7xl mx-auto flex items-center justify-center">
                  ...
                  <div className="p-6 border-t space-y-4 bg-muted/20">
                    <SheetClose asChild>
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-semibold">Podpora</p>
                          <p className="text-xs text-green-600 dark:text-green-500">Volat dispečink</p>
                        </div>
                      </a>
                    </SheetClose>
          ...
          {/* Center: Branding (Hidden to match client's clean look) */}
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 z-20">
            <Link
              ...
              className={cn(
                ...
                location.pathname === '/cleaner/profile' && "bg-primary/5"
              )}
            >
