...
import {
    ...
    CheckCircle2, MoreVertical, Star, ChevronDown, ChevronUp, Users
} from 'lucide-react';
...
export function BookingCard({ booking, onRatingSubmit, currentLoyaltyPoints, isCollapsible = false }: BookingCardProps) {
    ...
    if (details?.manual_loyalty_points) {
        singlePoints = details.manual_loyalty_points;
    } else {
        const price = booking.invoice?.total || details?.priceEstimate?.price || details?.priceEstimate?.priceMin || 0;
        singlePoints = Math.round(price * 0.27);
    }
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1.5 flex-1">
                            <h3 className="text-xl font-bold text-foreground/90 tracking-tight leading-none">
                                {details?.service_title || booking.service_type}
                            </h3>
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <div className="flex items-center gap-2">
                                <Badge variant="secondary" className={cn("px-3 py-1 text-[11px] font-bold rounded-full border shadow-sm whitespace-nowrap capitalize", displayStatus.color)}>
                                    {displayStatus.label}
                                </Badge>
                            </div>
                        </div>
                    ...
                    <CollapsibleContent className="space-y-6 pt-2 animate-in fade-in slide-in-from-top-4 duration-500">
                        ...
                        {!isDeclined && (
                            <div className="flex flex-col gap-4">
                                {/* 1. Price Container */}
                                {(details?.priceEstimate || (isCompleted && booking.invoice)) && (
                                        ...
                                        <div>
                                            ...
                                            <p className="text-lg font-bold">
                                                {isCompleted && booking.invoice ? (
                                                    `${booking.invoice.total.toLocaleString('cs-CZ')} Kč`
                                                ) : (
